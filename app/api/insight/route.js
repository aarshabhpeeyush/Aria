...
Profile: goal="${profile.goal}", diet="${profile.diet}", usual sleep="${profile.sleep}", activity="${profile.activity}", location/age="${profile.age_location || ''}"${conditions.toLowerCase() !== 'none' ? `, health notes="${conditions}"` : ''}
...
RULES: Never recommend medicines or supplements. Never suggest foods that conflict with their health notes/allergies. Write 2 sentences. Reference trends when meaningful. Warm, direct, specific. No greeting. Under 55 words.`
